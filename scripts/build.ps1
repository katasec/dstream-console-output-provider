...
switch ($Task) 
{
    'publish' {
        ...
        & $dotnetCmd publish console-output-provider.csproj -c Release -r $rid -p:DebugType=none -o out
        # dotnet publish console-output-provider.csproj -c Release -r win-x64 -p:DebugType=none -o out
    }
    'clean' {
        Write-Host '→ Cleaning…'
